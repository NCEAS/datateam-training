...
eml_parent <- read_eml(getObject(adc_test, child_pkg_1$metadata))
```
...
```{r, eval = FALSE}
eml_parent$dataset$title <- 'Time series of water temperature, specific conductance, and oxygen from Lake E1, North Slope, Alaska, 2012-2014'
```
...
eml_parent$dataset$coverage$temporalCoverage$rangeOfDates$endDate$calendarDate <- new_end_date
```
...
```{r, eval = FALSE}
eml_parent$dataset$dataTable <- NULL
```
