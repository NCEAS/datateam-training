...
Next, we will tell Git to store your PAT (we'll do that next, don't worry). We must do this because of the way  our server operating system handles credentials. If you don't run the next line, your PAT will expire immediately on the server, even though we will set it up on GitHub to be valid for 90 days.
...
If you are using the sasap-data repository, the URL is <http://github.nceas.ucsb.edu/NCEAS/sasap-data/>.
