...
Throughout this training we will occasionally use the syntax `package_name::function_name()`. This training covers several different R packages, so although you are not required to use the `package_name::` when calling a function, this syntax is used at times to make it clear which package a function comes from.
