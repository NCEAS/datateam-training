...
<!--chapter:end:01_trainingPkg.Rmd-->
...
<!--chapter:end:02_introArcticdatautils.Rmd-->

---
title: "Practical EML in R"
author: "Jeanette Clark"
date: "2/9/2017"
...
```{r}
path1 <- '~/arctic-data/docs/training/EML/Example_EML.xml'
eml <- read_eml(path1)
```
...
![RStudio Autocompletion](rstudio-autocomplete.png)
...
Be very careful about what you publish on the production node (PROD, or arcticdata.io). This node should NEVER be used to publish test or training datasets. While going through training for the first time you should be using the test environment (STAGING, or test.articdata.io).
...
## For files that aren't uploaded yet
...
# For files that are already uploaded
...
keywords <- new('keywordSet', 
                keywordThesaurus = 'GCMD',
                keyword = c('keyword1', 'keyword2', 'keyword3'))
eml@dataset@keywordSet <- c(keywords)
```
...
<!--chapter:end:03_editingEMLinR.Rmd-->
