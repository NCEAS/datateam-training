...
- **Data file names:** clear, but short names without blank spaces nor special characters (include extension)
...
- `entityDescription`s are set in all `dataTable`s and `otherEntity`s and EACH ONE IS UNIQUE so that they can be easily differentiated
...
Here's an example: `Terse methods section. See [child citation] for more information.`
...
- Set any versions that are in the middle of a [PublicDOI] -> [PrivateUUID] -> [PublicDOI] sandwich, to public. This goes for XML, resource maps, and data objects.
