...
To set the rights and access for all of the objects in a [package](#get-package-and-eml), first get the ORCiD of the person to whom you are giving rights and access. You can set this manually, or grab it from one of the creators in an EML file. You can look up ORCID iDs [here](https://orcid.org/orcid-search/search). 
...
### My profile
The datasets that render under a user's profile page (https://arcticdata.io/catalog/profile/http://orcid.org/0000-0003-3644-7237) are added if one of the following three System Metadata fields exists.  The subject is the `rightsHolder` or the subject has one of either `write` or `changePermission` in the `accessPolicy`.  
