...
- **Descriptive title:** what, where, when - provides enough information to understand the contents at a general scientific level (only first word and proper nouns capitalized, as in a journal article title; do not add a period at the end, it will auto-populate)
    - Ex: "Active layer soil bulk density, moisture, carbon and nitrogen concentration and stable isotope data from Alaska borehole sites, 2015"
...
- **Cell etiquette:** only one value (piece of information) per cell (for lat/long use decimal degrees) - `attributeList` should define ALL codes used (defining missing value codes is essential)
...
- **Creator and contact roles** are complete
      ...
      in the `userId` slots. Note the "s" in "https". See how to do this in an automated fashion [here](https://github.nceas.ucsb.edu/KNB/arctic-data/blob/master/datateam/How_To/EML/editingEMLinR.Rmd)  
      - Use the "references" tag to copy information for the same individual to multiple "party type"s ("creator", "contact", "associatedParty"), [like so](https://github.nceas.ucsb.edu/KNB/arctic-data/blob/master/datateam/How_To/EML/working-with-eml-references.md)  
      - Emails are important, phone #s and addresses are less critical

- **Geographic and temporal ranges** are sensible (start date preceeds end date) and appropriate to the study site and period; geogrpahic description is accurate
...
- After approval and DOI'ing, log out to ensure the landing page looks correct to average users. If there are discrepencies, ensure all versions (of all objects) following a public version, are also public.
