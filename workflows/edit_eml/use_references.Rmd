...
```{r, eval = FALSE}
eml@dataset@contact[[1]] <- eml_set_reference(element_to_reference = eml@dataset@creator[[1]],
                                              element_to_replace = eml@dataset@contact[[1]])
```
...
```{r, eval = FALSE}
eml <- eml_set_shared_attributes(eml, attributeList = attribute_list, type = 'dataTable')
```
...
# Use references to add updated contact info to Bryce's other roles
eml@dataset@contact[[1]] <- new('contact', reference = "bryce")
eml@dataset@metadataProvider[[1]] <- new('metadataProvider', reference = "bryce")
```

```
The following entry has been changed:<creator id="bryce" system="uuid">
  <individualName>
    <givenName>Bryce</givenName>
    <surName>Mecum</surName>
  </individualName>
  <userId directory="https://orcid.org">https://orcid.org/0000-1234-5678-4321</userId>
</creator>
```
