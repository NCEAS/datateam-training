...
Attributes can exist in an EML `eml@dataset@dataTable` or an `eml@dataset@otherEntity` object.  We prefer to store them in `dataTable` objects. Please note that submitting attribute information through the website will store them in an `otherEntity` by default.

Note, to edit/examine an existing attribute table already in an EML file you can use the following commands: 

```{r, eval = FALSE}
# If they stored in an otherEntity (submitted from the website by default)
...
You can create attributes by hand by typing them out in R following a workflow similar to the one below.
...
Note, `datamgmt::create_attributes_table` calls `datamgmt::get_custom_units` for you!

`datamgmt::get_custom_units` will auto-generate many of these fields for you (**but don't just assume the auto-generation will be perfect; always ensure the auto-generation correctly handles your unit**.)
