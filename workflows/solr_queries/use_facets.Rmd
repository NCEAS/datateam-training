...
We can also summarize what's in Solr with [faceting](https://lucene.apache.org/solr/guide/7_3/faceting.html), which lets us **group** Solr documents together and **count** them. This is like `table` in R. Faceting can do a query within a query, but more commonly it's used to summarize unique values in a field. For example, we can find the unique format IDs on Data Objects:
...
For additional ways to use faceting (such as pivot faceting), check out the [Solr documentation](https://lucene.apache.org/solr/guide/7_3/the-stats-component.html#the-stats-component).
```