...
result <- query(adc_prod, list(q="id:arctic-data.6*",
                               rows="5000"),
                as = "data.frame")
```
...
```{r eval = FALSE}
result <- query(adc_prod, list(q="title:soil*+AND+origin:Ludwig",
                               rows="5000"),
                as = "data.frame")
```

```{r eval = FALSE}
result <- query(adc_prod, list(q="title:soil* OR origin:Ludwig",
                               rows="5000"),
                as = "data.frame")
```
...
```{r, eval = FALSE}
result <- query(adc_prod, list(q = 'submitter:"http://orcid.org/0000-0003-4703-1974"',
...
```{r eval = FALSE}
result <- query(adc_prod, list(q="title:(soil* AND carbo*)",
                               rows="5000"),
                as = "data.frame") 
```
...
```{r eval = FALSE}
result <- query(adc_prod, list(q="title:(soil AND -carbon)",
                               rows="5000"),
                as = "data.frame")
```
...
```{r eval = FALSE}
result <- query(cn_prod, list(q="title:soil* AND origin:Ludwig",
                              rows="5000"),
                as = "data.frame")
```
